...
\begin{frame}{Early-Exercise Premium vs.\ Moneyness}
  ...
  \begin{columns}[T]
    \column{0.52\textwidth}
      \begin{tikzpicture}
        \begin{axis}[
          ...
          % Intrinsic value (payoff at maturity): max(K - S, 0)
          \addplot[thick, color=btgreen, dashed, domain=75:105, samples=100]
            {105 - x};
          \addplot[thick, color=btgreen, dashed, domain=105:125, samples=5]
            {0};
          \addlegendentry{Intrinsic value $\max(K{-}S,0)$}
          % European put (BS)
    ...
    \column{0.46\textwidth}
      ...
      \bigskip
      \footnotesize
      The \textcolor{btgreen}{dashed line} is the intrinsic value
      $\max(K{-}S_0,0)$---the payoff if exercised immediately.
      Both option prices lie \emph{above} it (time value $\ge 0$),
      except the American put at $S_0=80$ where
      immediate exercise is already optimal.
      Premium is largest when the option is \textbf{deep in-the-money}:
      receiving $K-S_0$ now and investing at $r$ outweighs waiting.
